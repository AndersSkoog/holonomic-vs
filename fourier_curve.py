...
"""
def radial_modulation_curve(dc,a,b,res,**kwargs):
  t_vals = np.linspace(0, 1, res)
  r_vals = np.array([fourier_series(dc, a, b, t) for t in t_vals])
  return [[r * cos(tau * t),r * sin(tau*t)] for r,t in zip(r_vals,t_vals)]

def angle_modulation_curve(dc,phi,psi,M,res,**kwargs):
  t_vals = np.linspace(0, 1, res)
  theta_vals = tau * M * t_vals + np.array([fourier_series(0, phi, psi, t) for t in t_vals])
  return [[dc * cos(theta), dc * sin(theta)] for theta in theta_vals]

def angle_and_radial_modulation_curve(dc,a,phi,b,psi,M,res,**kwargs):
    t_vals = np.linspace(0, 1, res)
    r_vals = np.array([fourier_series(dc,a,b,t) for t in t_vals])
    theta_vals = tau * M * t_vals + np.array([fourier_series(0, phi, psi, t) for t in t_vals])
...
if __name__ == "__main__":
  ...
  args = {"dc":0.6,"a":[0.2,0.5,0.7,0.3],"b":[0.9,0.4,0.1,0.7],"phi":[0.4,0.7,0.2,0.7],"psi":[0.7,0.2,0.34,0.73],"M":1,"res":360}
